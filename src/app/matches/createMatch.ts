...
export default async function createMatch(formData: FormData) {
  ...
  if ( !contestantDataA.success ) 
    return reportError(contestantDataA.error.flatten().fieldErrors);
  if ( !contestantDataB.success ) 
    return reportError(contestantDataB.error.flatten().fieldErrors);
  ...
  const buildContestant = async (data: typeof contestantDataA.data) => {
    ...
    const contextProvider = await prisma.contextProvider.findUniqueOrThrow({ where: { id: data.contextProvider } });
    return prisma.contestant.create({data: { ai: { connect: {id: ai.id} }, project: { connect: { id: project.id } }, contextProvider: { connect: { id: contextProvider.id } } }})
  }

  const [contestantA, contestantB] = await Promise.all([buildContestant(contestantDataA.data), buildContestant(contestantDataB.data)]);
  const match = await prisma.match.create({ data: { contestants: { connect: [{ id: contestantA.id }, { id: contestantB.id }] } } });
